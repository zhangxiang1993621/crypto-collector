...
- `find-polluter.ps1` 是 `find-polluter.sh` 的 PowerShell 移植版（在 DSH `pwsh` 工具里用 `&` 调用）：
  `& .dsh/skills/systematic-debugging/find-polluter.ps1 -PollutionCheck ".git" -TestPattern "src/**/*.test.ts" [-TestCommand "npm test"]`
